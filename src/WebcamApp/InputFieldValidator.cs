...
public class InputFieldValidator
{
    ...
    private readonly Regex _houseNumberRegex = new(@"^\d+[a-zA-Z]{1}$");
    ...
    public bool IsAnyAddressPropertyFilled(Panel panel, HashSet<string> excludedTextBoxNames) // Return true if any address property is filled
    {
    ...
    public string? ValidateTextBoxContent(Panel panel, string countryTextBox, string houseNumberTextBox, string? emailTextBox, string cityTextBox) // Return a list of missing or invalid fields
    {
        ...
        foreach (var textBox in panel.Controls.OfType<TextBox>())
        {
            ...
            else if(textBox.Name == cityTextBox && !string.IsNullOrWhiteSpace(textBox.Text) && !_cityRegex.IsMatch(textBox.Text))
            {
                invalidStringFields.Add(textBox.Tag.ToString()!);
            }
        }
        ...
        if (invalidStringFields.Any())
            errorMessages.Add($"Please enter valid string literals (only alphabets) for the following fields: {string.Join(", ", invalidStringFields)}");
